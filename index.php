...
try{
    $sql =" SELECT news.id, news.newsDate,news.title,news.content,news.active, news.image, categorytable.categoryName FROM `news` INNER JOIN categorytable ON news.category=categorytable.id  WHERE news.active=1  ORDER BY ID DESC  limit 4 ";
...
<body>
    <!-- Topbar Start -->
   <?php include_once("admin/includes/topBar.php");?>
    <!-- Topbar End -->
                        ...
                        <div class="col-lg-6">
                         <?php
                            foreach($stmt->fetchAll() as $row){
                                $id = $row["id"];
                                $Date = $row["newsDate"];
                                $NewsDate = date("M d, Y",strtotime($Date));
                                $Title = $row["title"];
                                $Content = $row["content"]; 
                            ...
                            ?>                      
                                    <div class="d-flex align-items-center bg-white mb-3" style="height: 110px;">
                                        ...
                                        <div class="w-100 h-100 px-3 d-flex flex-column justify-content-center border border-left-0">
                                            <div class="mb-2">
                                                <a class="badge badge-primary text-uppercase font-weight-semi-bold p-1 mr-2" href=""><?php echo $Category ?></a>
                                                <a class="text-body" href=""><small><?php echo $NewsDate ?></small></a>
                                            </div>
